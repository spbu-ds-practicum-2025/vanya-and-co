...
| **Владалец** | пользователь, загрузивший файл. Имеет полные права (изменение, удаление) |
| **ACL** | передача файлов между пользователями |
...
2. Пользователь вводит личныне данные.
3. Пользователь попадает в личный кабинет, получает доступ к интерфейсу.

### Сценарий: загрузка файла
1. Пользователь выбирает загрузку файла.
2. На экране пользователя появляется окно выбора с возможностью "перетащить файл", "загрузить файл с диска", "загрузить файл из буфера обмена".
...
5. Файл скаявается на диске у пользоваетля.
